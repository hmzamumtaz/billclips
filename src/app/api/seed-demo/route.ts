...
export async function POST() {
  try {
    ...
    const { data: existing } = await supabase.from("invoices").select("id").eq("client_email", DEMO_EMAIL).limit(1);
    ...
    const { error: profileError } = await supabase.from("users").upsert({ id: userId, email: DEMO_EMAIL, full_name: "Demo User" }, { onConflict: "id" });
    ...
    const invoiceRows = clients.map((c, i) => {
      ...
      return { user_id: userId, client_name: c.name, client_email: c.email, client_phone: c.phone, client_address: c.address, amount_cents: c.amount, due_date: d.toISOString().split("T")[0], status: c.status, notes: `Sample invoice for ${c.name}` };
    });
    ...
    const { data: seq, error: seqErr } = await supabase.from("sequences").insert({
      user_id: userId, name: "Standard Reminder Flow", description: "Gentle at 3 days, escalate at 7, final at 14", applies_to_status: "overdue", is_active: true,
    }).select().single();
    ...
    return Response.json({ message: "Demo account created", email: DEMO_EMAIL, password: DEMO_PASSWORD });
  } catch (err) {
